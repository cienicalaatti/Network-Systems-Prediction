...
def linked_lorenz2(NE, z, a, n_timesteps, h_range = 0.06, h = 0.9, rho = 28.0, beta = 2.667, sigma = 10.0, sigma_dyn = 0.1)  -> Tuple[np.ndarray, np.ndarray]:
    ...
    def linked_lorenz_diff(t, z):
        ...
        for k in range(NE):
                # dx[k]/dt:
                for l in range(NE):
                    dz_dt[3*k] += a[k][l]*(z[3*l+1])
                dz_dt[3*k] = -sigma*(z[3*k] -z[3*k+1]) + dz_dt[3*k]    
                # dy[k]/dt:
